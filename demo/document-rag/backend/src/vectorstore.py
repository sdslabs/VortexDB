...
class VectorStore:
    ...
    def health_check(self) -> None:
        ...
        if response.status_code != 200:
            raise Exception(f"VortexDB health check failed: {response.text}")

    def insert_batch(
        self, vectors: List[List[float]], texts: List[str], filename: str
    ) -> int:
        ...
        for i, (vector, text) in enumerate(zip(vectors, texts)):
            points.append(
                {"vector": vector, "payload": {"content_type": "Text", "content": text}}
            )
    ...
    def search(self, query_vector: List[float], top_k: int = 5) -> List[Dict]:
        ...
        response = client.post(
            "/points/search",
            json={"vector": query_vector, "similarity": "Cosine", "limit": top_k},
        )
        ...
        for point_id in data.get("results", []):
            ...
            if point_response.status_code != 200:
                raise Exception(
                    f"Failed to fetch point {point_id}: {point_response.text}"
                )
