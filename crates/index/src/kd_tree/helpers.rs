...
pub fn is_unbalanced(node: &KDTreeNode) -> bool {
    ...
    max_child as f32 > BALANCE_THRESHOLD * node.subtree_size as f32
}
...
pub fn collect_recursive(node: KDTreeNode, result: &mut Vec<IndexedVector>) {
    ...
    if let Some(left) = node.left {
        collect_recursive(*left, result);
    }
    if let Some(right) = node.right {
        collect_recursive(*right, result);
    }
}
...
pub fn should_rebuild_global(total_nodes: usize, deleted_count: usize) -> bool {
    total_nodes > 0 && (deleted_count as f32 / total_nodes as f32) > DELETE_REBUILD_RATIO
}
